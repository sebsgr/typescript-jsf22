...
gulp.task('js-es6', () => {
    return rollup({
        ...
        return bundle.write({
            file: './reveal-js/dist/reveal.esm.js',
            format: 'es',
            banner: banner,
            sourcemap: true
        });
...
function compileSass() {
  return through.obj( ( vinylFile, encoding, callback ) => {
    ...
    sass.render({
        ...
        includePaths: ['reveal-js/css/', 'reveal-js/css/theme/template']
    }, ( err, result ) => {
        if( err ) {
...
gulp.task('css-themes', () => gulp.src(['./reveal-js/css/theme/source/*.{sass,scss}'])
        .pipe(compileSass())
        .pipe(gulp.dest('./reveal-js/dist/theme')))
...
gulp.task('eslint', () => gulp.src(['./js/**','./aformatik/js/**','./reveal-js/js/**', 'gulpfile.js'])
        .pipe(eslint())
        .pipe(eslint.format()))
...
gulp.task('serve', () => {
    ...
    gulp.watch(['*.html','slides/*.html', '*.md'], gulp.series('reload'))

    gulp.watch(['reveal-js/js/**'], gulp.series('js', 'reload', 'eslint'))

    gulp.watch(['reveal-js/plugin/**/plugin.js', 'reveal-js/plugin/**/*.html'], gulp.series('plugins', 'reload'))
